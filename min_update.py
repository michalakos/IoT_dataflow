...
if len(sys.argv) != 2:
    print("Usage: ./min_update.py sensors_count")
    quit()

SENSOR_COUNT = sys.argv[1]

min_consumer = KafkaConsumer(
    'min',
...
# create table to store min values for each sensor
# every row has the data's timestamp as a key and min_value as column
for sensor in sensors:
    conn.create_table(sensor+'_min_values', families)
    tables[sensor] = conn.table(sensor+'_min_values')

# wait for Kafka data and store in Hbase
for message in min_consumer:
